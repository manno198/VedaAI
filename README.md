...
- `pdfjs-dist` + `@napi-rs/canvas` (called directly, `lib/pdf.ts`) to rasterize PDF pages to PNGs server-side, since vision models take images, not PDFs
...
## Why pdfjs-dist is called directly

This app originally used `pdf-to-img`, a thin convenience wrapper around `pdfjs-dist` + `@napi-rs/canvas`. It worked in local dev but broke twice in a row on Vercel with two different `Cannot find module` errors — first `@napi-rs/canvas`'s `DOMMatrix`/`ImageData`/`Path2D` polyfills, then `pdfjs-dist/package.json` itself. Both traced back to the same root cause: `pdf-to-img` locates these files at runtime via `createRequire(import.meta.url).resolve(...)` — a dynamically-constructed require that Next.js's build-time file tracer can't follow, so Vercel's serverless bundler (which only ships a traced subset of `node_modules` per function, not the whole tree) silently leaves them out. This is a confirmed, open, unresolved upstream bug: [k-yle/pdf-to-img#259](https://github.com/k-yle/pdf-to-img/issues/259).

Rather than keep patching `outputFileTracingIncludes` glob-by-glob as each nested file surfaced, `lib/pdf.ts` now calls `pdfjs-dist`'s legacy Node build directly, with `pdfjs-dist` and `@napi-rs/canvas` as this project's own **direct** dependencies (pinned to the exact versions `pdf-to-img@6.3.0` used internally — `pdfjs-dist@5.6.205`, `@napi-rs/canvas@0.1.100` — so nothing about the actual rendering behavior changed, only how the files are located). Font/cmap data paths are computed from `process.cwd()` rather than a runtime module resolution, sidestepping the fragile lookup entirely. `pdfjs-dist`'s own internal Node canvas factory still resolves `@napi-rs/canvas` the same dynamic way internally — that one native binary is covered by `next.config.mjs`'s `outputFileTracingIncludes`, now a simple top-level glob instead of reaching two directories into another package's own `node_modules`.

This was verified by more than a passing build: the exact traced-files mechanism Vercel uses (`.next/standalone`, Next's `output: "standalone"` mode) was built locally, copied to a directory with zero access to the project's real `node_modules`, and run end-to-end against real Gemini/Groq calls with real PDFs and images — see the Approach section's pipeline diagram for what "end-to-end" covers.
...
2. **Convert** — every file is rasterized to PNG pages (`lib/pdf.ts`). Images pass through unchanged; PDFs are rendered page-by-page by calling `pdfjs-dist`'s legacy Node build directly (its own built-in `NodeCanvasFactory` handles the actual drawing via `@napi-rs/canvas`) — not through the `pdf-to-img` wrapper package, which has an [open, unresolved bug](https://github.com/k-yle/pdf-to-img/issues/259) resolving its own `pdfjs-dist/package.json` path on Vercel. Pages are numbered continuously if a document is uploaded as multiple files.
...
- **`pdfjs-dist` (now a direct dependency — see "Why pdfjs-dist is called directly" above) has an open advisory** for arbitrary JS execution when opening a maliciously crafted PDF (`GHSA-hq66-cqwq-w95j`, affects `>=5.6.83 <6.2.108`; this project is pinned to `5.6.205`). Since this app rasterizes user-uploaded PDFs server-side, that's a real consideration for a production deployment. A fix is available (`pdfjs-dist@6.2.108`) but is a major version bump — deliberately not taken here right after stabilizing the Vercel deployment (see "Why pdfjs-dist is called directly" earlier in this README), but it's a concrete, low-effort next step worth doing deliberately rather than folded into an unrelated change.

---

Made by Harshita Singh.

I used Claude Code as a supporting tool during development for understanding concepts, debugging, and getting guidance on implementation decisions. The application, architecture, and final implementation were developed and integrated by me.
